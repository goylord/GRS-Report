  ...
  <div class="formComponent">
    <el-form ref="form" :model="values" label-width="80px">
      <el-form-item v-for="(formItem, index) in props.formModel" :label="formItem.fieldName" :key="index">
        <el-input v-if="formItem.fieldType === 'input'" v-model="values[formItem.fieldCode]" placeholder="请输入"></el-input>
        <el-input-number v-else-if="formItem.fieldType === 'number'" v-model="values[formItem.fieldCode]" placeholder="请输入"></el-input-number>
        <el-switch v-else-if="formItem.fieldType === 'switch'" v-model="values[formItem.fieldCode]"></el-switch>
